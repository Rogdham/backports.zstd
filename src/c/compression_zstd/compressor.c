...
{
    assert(BACKPORTSZSTD_PyMutex_IsLocked(&self->lock));
    PyObject *level = NULL;
...
_zstd_load_c_dict(ZstdCompressor *self, PyObject *dict)
{
    ...
    if (ret < 0) {
        ...
        BACKPORTSZSTD_PyMutex_Lock(&zd->lock);
        ret = _zstd_load_impl(self, zd, mod_state, type);
        BACKPORTSZSTD_PyMutex_Unlock(&zd->lock);
        return ret;
    }
...
/*[clinic end generated code: output=cdef61eafecac3d7 input=92de0211ae20ffdc]*/
{
    ...
    self->dict = NULL;
    self->lock = BACKPORTSZSTD_PyMutex_allocate();
    if(BACKPORTSZSTD_PyMutex_IsNull(self->lock)) {
        _zstd_state* mod_state = PyType_GetModuleState(Py_TYPE(self));
        if (mod_state != NULL) {
            PyErr_SetString(mod_state->ZstdError,
...
ZstdCompressor_dealloc(PyObject *ob)
{
    ...
    assert(!BACKPORTSZSTD_PyMutex_IsLocked(&self->lock));
    BACKPORTSZSTD_PyMutex_free(self->lock);
...
/*[clinic end generated code: output=ed7982d1cf7b4f98 input=ac2c21d180f579ea]*/
{
    ...
    /* Thread-safe code */
    BACKPORTSZSTD_PyMutex_Lock(&self->lock);
    ...
    BACKPORTSZSTD_PyMutex_Unlock(&self->lock);
